...
En mi actual versión sólo para móviles, tengo distintas pantallas para ejecutar una tarea. La meta de una pantalla exclusiva sólo para iPad es colocar una lista de recursos a la izquierda (peticiones HTTP en este caso) y el contenido al lado derecho.

Ya que estoy usando **React Native Navigation**. La única solución que pensé es crear una pantalla personalizada sólo para iPad. Pero, ¿puede React Native detectar un dispositivo iPad? Bueno, he aquí la solución.

## El valor de isPad

Este valor sólo está disponible si `Platform` detecta que es un dispositivo iOS. Así que ten cuidado con eso y agrega las respectivas validaciones.
