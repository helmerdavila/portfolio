...
Et voilà les **\***spies**\*** de Jest nos aident. Même si `isIpad`est une fonction **\*\***getter,**\*\*** on peut le replacer. J’utilise `mockReturnValueOnce`pour spécifier le retour de la fonction la première fois qui est exécuté.

> En français, `mockReturnValueOnce`, serait traduit par `faireMockRetournerUneFois`

Pout Typescript, on ajoute `PlatformIOSStatic`, c’est une façon de dire en TS que les **\***types**\*** de `Platform` se réfèrent aux appareils iOS et iPad.
